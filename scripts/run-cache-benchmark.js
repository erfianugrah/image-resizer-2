 ...
 * 
 * This script runs cache performance benchmarks specifically and generates a report.
 * It can be used to compare default vs. optimized caching strategies.
 */
...
const reportFile = path.join(REPORT_DIR, `cache-benchmark-report-${timestamp}.json`);

console.log('🚀 Starting cache performance benchmark');
console.log(`📊 Iterations: ${iterations}, Warmup: ${warmup}`);

// Run the benchmark using vitest
const result = spawnSync('npx', ['vitest', 'run', 'test/performance/cache-benchmark.ts', '--reporter', 'json'], {
...
if (saveReport && testResults) {
        ...
        testFile.assertionResults.forEach(assertion => {
          ...
          if (assertion.status === 'passed' && assertion.title.includes('benchmark')) {
            ...
            consoleOutput.forEach(log => {
              ...
              } else if (log.message && log.message.includes('Default Cache Service - Bypass Decision')) {
                timingData.defaultBypass = parseFloat(log.message.match(/Average time: ([\d.]+)ms/)[1]);
              } else if (log.message && log.message.includes('Optimized Cache Service - Bypass Decision')) {
                timingData.optimizedBypass = parseFloat(log.message.match(/Average time: ([\d.]+)ms/)[1]);
              } else if (log.message && log.message.includes('Default Cache Service - Apply Headers')) {
                timingData.defaultHeaders = parseFloat(log.message.match(/Average time: ([\d.]+)ms/)[1]);
              } else if (log.message && log.message.includes('Optimized Cache Service - Apply Headers')) {
                timingData.optimizedHeaders = parseFloat(log.message.match(/Average time: ([\d.]+)ms/)[1]);
              } else if (log.message && log.message.includes('End-to-End Default Cache Average')) {
                timingData.defaultEndToEnd = parseFloat(log.message.match(/Average: ([\d.]+)ms/)[1]);
              } else if (log.message && log.message.includes('End-to-End Optimized Cache Average')) {
                timingData.optimizedEndToEnd = parseFloat(log.message.match(/Average: ([\d.]+)ms/)[1]);
              } else if (log.message && log.message.includes('End-to-End Cache Performance improvement')) {
  ...
  console.log(`✅ Cache benchmark report saved to ${reportFile}`);
  ...
  if (htmlResult.status === 0) {
    console.log(`✅ HTML report generated: ${reportFile.replace('.json', '.html')}`);
  } else {
