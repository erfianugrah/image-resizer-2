...
export class DefaultCacheService implements CacheService {
      ...
      const cacheOperation = async (): Promise<Response> => {
        ...
        try {
          ...
          if (cachedResponse.status >= 200 && cachedResponse.status < 300) {
            ...
            const responseToCache = this.prepareCacheableResponse(
              cachedResponse,
            );
            ...
            const requestWithTags = this.prepareTaggedRequest(
              request,
              responseToCache,
            );
  ...
  generateCacheTags(
    ...
    try {
      ...
      if (!request || !isRequest(request)) {
        throw new CacheServiceError(
          "Invalid request object provided to generateCacheTags",
          {
            code: "INVALID_REQUEST",
            status: 500,
            details: {
      ...
      if (!storageResult) {
        throw new CacheTagGenerationError(
          "Missing storage result for tag generation",
          {
            details: {
      ...
      try {
        ...
        throw new CacheTagGenerationError(
          "Invalid URL in request for tag generation",
          {
            details: {
              url: request.url,
              error: urlError instanceof Error
      ...
      try {
        ...
        const leadingSlashPattern =
          config.cache.cacheTags?.pathNormalization?.leadingSlashPattern ||
          "^/+";
        const invalidCharsPattern =
          config.cache.cacheTags?.pathNormalization?.invalidCharsPattern ||
          "[^a-zA-Z0-9-_/.]";
        const replacementChar =
          config.cache.cacheTags?.pathNormalization?.replacementChar || "-";
        ...
        tags.push(
          `${prefix}path-${normalizedPath.join("-").replace(/\./g, "-")}`,
        );
        ...
        normalizedPath.forEach((segment, index) => {
          ...
          if (normalizedPath.length > 1) {
            ...
            tags.push(
              `${prefix}segment-${index}-${segment.replace(/\./g, "-")}`,
            );
          }
        });
      } catch (pathError) {
        this.logger.warn("Error processing path for cache tags", {
          error: pathError instanceof Error
            ? pathError.message
            : String(pathError),
          imagePath,
        });
      ...
      if (
        config.cache.cacheTags?.customTags &&
        Array.isArray(config.cache.cacheTags.customTags)
      ) {
        this.logger.breadcrumb(
          "Adding custom tags from configuration",
          undefined,
          {
        ...
        config.cache.cacheTags.customTags.forEach((tag: string) => {
          ...
          const safeTag = tag.replace(
            new RegExp(invalidCharsPattern, "g"),
            replacementChar,
          );
      ...
      if (config.cache.cacheTags?.pathBasedTags) {
        this.logger.breadcrumb("Processing path-based tags", undefined, {
          pathBasedTagsCount:
            Object.keys(config.cache.cacheTags.pathBasedTags).length,
        });

        const pathBasedTags = config.cache.cacheTags.pathBasedTags as Record<
          string,
          string[]
        >;
        const invalidCharsPattern =
      ...
      if (
        ...
        const excludeHeaders = metadataConfig.excludeHeaders ||
        ...
        storageResult.response.headers.forEach((value, key) => {
          ...
          const isMetadataHeader = headerPrefixes.some((prefix: string) =>
            headerKey.startsWith(prefix)
          );
        ...
        if (metadataConfig.includeContentType) {
          const contentType = storageResult.response.headers.get(
            "content-type",
          );
        ...
        if (metadataConfig.includeCacheControl) {
          const cacheControl = storageResult.response.headers.get(
            "cache-control",
          );
      ...
      if (
        ...
        typeof storageResult.response.status === "number"
      ) {
        const status = storageResult.response.status;
        const statusCategory = status >= 200 && status < 300
          ? "success"
      ...
      if (contentType) {
        try {
          ...
          this.logger.warn("Error parsing content type for cache tags", {
            contentType,
            error: contentTypeError instanceof Error
      ...
      if (storageResult.size) {
        try {
          ...
          this.logger.warn("Error categorizing size for cache tags", {
            ...
            error: sizeError instanceof Error
              ? sizeError.message
              : String(sizeError),
          });
          // Continue without these tags rather than failing completely
      ...
      if (storageResult.width && storageResult.height) {
        try {
          ...
          this.logger.warn("Error categorizing dimensions for cache tags", {
            ...
            error: dimensionError instanceof Error
      ...
      this.logger.debug("Generated cache tags", {
        ...
        sampleTags: tags.slice(0, 5).join(", ") +
          (tags.length > 5 ? "..." : ""),
      });
      ...
      const processedError = this.handleError(
        ...
        "Generate cache tags",
        request,
        "CACHE_TAG_GENERATION_ERROR",
      );

      // Convert any CacheServiceError to a specific CacheTagGenerationError
      const message =
        `Failed to generate cache tags: ${processedError.message}`;
  ...
  applyCloudflareCache(
    ...
    try {
      ...
      this.logger.breadcrumb(
        "Checking cache tag generation requirements",
        undefined,
        {
          cacheTagsEnabled,
        ...
        try {
          ...
          if (cacheTags.length > 0) {
            this.logger.debug("Generated cache tags for Cloudflare fetch API", {
              ...
              sampleTags: cacheTags.slice(0, 3).join(", ") +
                (cacheTags.length > 3 ? "..." : ""),
            });
          }
        } catch (tagError) {
      ...
      if (
        ...
        this.logger.breadcrumb(
          "Using cacheTtlByStatus for granular cache control",
          undefined,
          {
      ...
      if (cacheTags.length > 0) {
        this.logger.debug("Adding cache tags to Cloudflare request CF object", {
          tagCount: cacheTags.length,
          firstFewTags: cacheTags.slice(0, 5).join(", ") +
            (cacheTags.length > 5 ? "..." : ""),
        });
      ...
      const endTime = Date.now();
      this.logger.breadcrumb(
        "Applied Cloudflare cache settings",
        endTime - startTime,
        {
    ...
    const primaryOperation = async () => {
      try {
        ...
        if (config.cache.enableStaleWhileRevalidate) {
          ...
          const staleResponse = await this.tryGetStaleResponse(
            request,
            ctx,
            options,
          );
        ...
        if (
          ...
          const cachedResponse = this.applyCacheHeaders(
            response,
            options,
            storageResult,
          );
    ...
    if (
      ...
      try {
        ...
        this.logger.warn("Failed to add resource hints", {
          error: hintError instanceof Error
            ? hintError.message
            : String(hintError),
          url: request.url,
        });
  ...
  calculateTtl(
    ...
    try {
      ...
      if (
        status >= 200 && status < 300 && config.cache.enableStaleWhileRevalidate
      ) {
        ...
        try {
          ...
          this.logger.warn("Failed to add stale-while-revalidate directive", {
            error: headerError instanceof Error
              ? headerError.message
              : String(headerError),
          });
